...
for(var i=0;i<$('.menu-catg li').length;i++){
     menu_i[i].addEventListener('click',function(){
        current_category=this.innerText;
      ...
      document.querySelector('.cat_branch').textContent=current_category;
...
$('.login_icon').on('click',function(){
  console.log('hello')
  $('.home_page_login').css('display','block');
...
$(document).click(function (e) { 
            if ($(e.target).closest(".home_page_login").length == 0 && $(e.target).closest(".login_icon").length == 0) {         
                  $('.home_page_login').css('display','none');              
            }}); 
